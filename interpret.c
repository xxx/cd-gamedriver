...
f_pcre_capture(int num_arg)
{
    ...
    struct vector *result = pcre_capture((sp-1)->u.string, re);
    pcre2_code_free(re);
...
{
    if (sp->type != T_STRING)
	bad_arg(3, F_WRITE_BYTES, sp);

    if (sp->u.string == NULL)
	error("Attempt to write empty string.\n");

    int i = write_bytes((sp-2)->u.string, (sp-1)->u.number, sp->u.string);
    pop_n_elems(3);
    push_number(i);
}
